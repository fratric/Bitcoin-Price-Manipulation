...
// [[Rcpp::export]]
List simulator_base(bool full_return, int Ndays, int Tt, double meanN, double varN, double a, double b, double pleft, double pright, int lShort, NumericMatrix matrixcash, double prob_drop, double pCA_long, double percent, double pRA, double pRSA, bool LSEbool) {
  ...
  //std::vector<double> orderSell_snap(0);
  //std::vector<double> orderBuy_snap(0);
  ...
  //current_price = Rcpp::rnorm(1,current_price, 20)[0];
  //orderbook random initialization
  int warm_up = 10;
  Rcpp::NumericVector warmUp_prices(warm_up);
  for(int day=0;day<warm_up;day++){
    
    //Fill_order_book(current_price, -1, 2*fill_size, dfSell_residual_amount, dfSell_limit_price, dfSell_isMMA, dfSell_expire);
    //Fill_order_book(current_price, 1, 2*fill_size, dfBuy_residual_amount, dfBuy_limit_price, dfBuy_isMMA, dfBuy_expire);
    for(int t=0;t<Tt;t++){
      ...
      if(current_price <= 1.0 || current_price > 100000){
        t = Tt;
        current_price = 0;
      }
    }    
  ...
  //NumericVector probs = NumericVector::create(pNA, pMR, pCA, pMMA);
  //simulation
  int index_bas = 0;
  int index_toProve = 0;
  int index_snaps = 0;
  ...
  for(int day=0;day<Ndays;day++){
    //Fill_order_book(current_price, -1, fill_size, dfSell_residual_amount, dfSell_limit_price, dfSell_isMMA, dfSell_expire);
    //Fill_order_book(current_price, 1, fill_size, dfBuy_residual_amount, dfBuy_limit_price, dfBuy_isMMA, dfBuy_expire);
    ...
    for(int t=0;t<Tt;t++){
      ...
      if(dfb_size > 0 && dfs_size > 0 && MMAdefault == false){
        ...
        if(market_exit(current_price) == true){
          cashM = 0.0;
        }
        ...
        if((int)to_issue["order_type"] != 0){
          ...
          if((int)to_issue["order_type"] == -1){
            //Rcout  << "after  price:" << current_price << "\n";
          }
        }
      }
      ...
      if(dfb_size > 0 && dfs_size > 0  && current_price > 1 && Rcpp::runif(1)[0] < pRSA){
        ...
        if((int)to_issue["order_type"] != 0){
          if((int)to_issue["order_type"] == 1){
            AppendOrderBuy(to_issue["order"], dfBuy_residual_amount, dfBuy_limit_price,  dfBuy_isMMA, dfBuy_expire);
            //activity_buy(3,day) = activity_buy(2,day) + 1.0/Tt;
          }else if((int)to_issue["order_type"] == -1){
            AppendOrderSell(to_issue["order"], dfSell_residual_amount, dfSell_limit_price, dfSell_isMMA, dfSell_expire);
            //activity_sell(3,day) = activity_sell(2,day) + 1.0/Tt;
          }
          List tmpList = keepMatching(current_price, dfSell_residual_amount, dfSell_limit_price, dfSell_isMMA, dfSell_expire, dfBuy_residual_amount, dfBuy_limit_price, dfBuy_isMMA, dfBuy_expire);
          current_price = tmpList["new_price"];
          double MMAincr = tmpList["BTC_transfered_MMA"];
      ...
      //Rcout << "day=" << day << "  t=" << t << " cp = " << current_price  << "\n";
      //print_orders(dfSell0, dfSell_residual_amount, dfSell_limit_price, dfSell_issue_time, dfSell_expiration_time);
      //print_orders(dfBuy0, dfBuy_residual_amount, dfBuy_limit_price, dfBuy_issue_time, dfBuy_expiration_time);
    ...
    dayIncrement(dfSell_expire);
    dayIncrement(dfBuy_expire);
    //Rcout << "p_day=" << current_price << " (" << day << "): " << dfSell_residual_amount.size() << " " << dfBuy_residual_amount.size() << "\n";
    ExpireOrders(order_book_max_len, dfSell_residual_amount, dfSell_limit_price, dfSell_isMMA, dfSell_expire);
    ExpireOrders(order_book_max_len, dfBuy_residual_amount, dfBuy_limit_price, dfBuy_isMMA, dfBuy_expire);
    ...
    for(int i=0; i<lShort;i++){ // I think this is here to calculate it from returns
  ...
  // print_vector(Market_volume);
  //print_vector(dfSell_limit_price);
  //print_vector(dfSell_residual_amount);
